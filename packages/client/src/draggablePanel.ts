/**
 * Shared drag/resize/persist logic for ChatPanel and NoteDialog.
 *
 * Each panel needs the same four things: an initial Rect (from
 ...
 * callback for the Rnd onDragStop / onResizeStop handlers.
 *
 * The default rect is a callback so it can read `window.inner*` lazily
 * — the hook is mounted client-side only, but the consumer can still
 * compose its own placement (e.g. anchor to right edge vs. center).
 */
...
function saveRect(storageKey: string, r: Rect): void {
  try {
    ...
    // localStorage may be disabled (private mode, quota); the panel still
...
export function useDraggablePanel(opts: DraggablePanelOptions): DraggablePanelApi {
  ...
  useEffect(() => {
    ...
    // opts is captured by closure; consumers pass a stable object literal
    // (the panel components define the constants at module scope).
